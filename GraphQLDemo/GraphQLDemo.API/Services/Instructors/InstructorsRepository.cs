...
namespace GraphQLDemo.API.Services.Instructors
{
    public class InstructorsRepository
    {
        ...
        public async Task<InstructorDTO> GetById(string instructorId)
        {
        ...
        public async Task<IEnumerable<InstructorDTO>> GetManyByIds(IReadOnlyList<string> instructorIds)
        {
            using (SchoolDbContext context = _contextFactory.CreateDbContext())
            {
